...
interface Props {
    ...
    /**
     * Tag ของข้อความที่ใส่ใน Slot "title"
     * ค่าเริ่มต้นคือ "h1"
     */
    ...
    <div>
        <header class="relative">
            ...
            <div class="absolute bottom-[7rem] px-5 md:px-[4.6rem] text-white">
                <component :is="titleAsTag" class="font-bold text-3xl sm:text-4xl lg:text-6xl leading-none"><slot name="title" mdc-unwrap="p" /></component>
                <p v-if="$slots['secondary-title']" class="font-bold text-lg lg:text-2xl">
                    <slot name="secondary-title" mdc-unwrap="p" />
                </p>
                <p v-if="$slots.description" class="font-light text-lg lg:text-2xl" name="description" mdc-unwrap="p">
