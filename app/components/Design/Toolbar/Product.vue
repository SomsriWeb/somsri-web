...
<template>
    <div class="w-screen h-[90vh] max-w-56 max-h-[22rem] bg-stone-100 rounded-lg p-3 shadow-lg space-y-3 overflow-auto">
        <div class="flex justify-between items-center gap-3">
            <ProseH2>ตั้งค่าสินค้า</ProseH2>
            <UButton size="sm" type="button" icon="i-material-symbols-close-rounded" color="error" variant="ghost" class="!min-w-fit" @click="open = false" />
        </div>

        <template v-if="!subComponentMenuState">
            <UFormField label="ประเภทสินค้า">
                <USelectMenu v-model="design.type" class="w-full" :items="products" value-key="value" @change="setToProductDefault()" />
            </UFormField>

            <UFormField label="ประเภทเนื้อผ้า">
                <USelectMenu v-model="design.fabric" class="w-full" :items="fabricOptions" value-key="value" />
            </UFormField>
            ...
            <div v-if="!isApron && !isPolo">
                ...
                <UCarousel v-slot="{ item }" :items="styleOptions" :ui="{ container: 'flex gap-3' }" class="rounded-lg overflow-hidden">
                    <button
                        class="flex flex-col justify-center items-center border-2 p-3 rounded-xl"
                        :class="[design.style === item.value ? activeStyleClass : nonActiveStyleClass]"
                        @click="selectStyle(item.value)"
                    >
                        <p class="font-medium text-primary-500">
                            {{ item.label }}
                        </p>
                        <img class="w-[4rem] h-[4rem] rounded-xl" :src="item.imgUrl" :alt="item.label" />
                    </button>
                </UCarousel>
            </div>
        </template>
